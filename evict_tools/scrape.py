...
class EvictionScraper:
    ...
    def scrape_by_name(self, query_args):
        """
        Attempt to identify an eviction case by searching for an applicant
        name. If a case exists, then get the hearing information.

        Args:
            query_args: dictionary of query arguments used to identify and
                verify a case by a name search
        ...
        soup = BeautifulSoup(page.text, "html.parser")
        case_id_elements = soup.find_all(
            "a",
            attrs={"href": re.compile("ck_public_qry_doct.cp_dktrpt_frames")},
        )
    ...
    def _clean_names(self, query_args):
        query_args["last_name"] = re.sub(
            r"\([^()]*\)", "", query_args["last_name"]
        ).strip()
        query_args["first_name"] = re.sub(
            r"\([^()]*\)", "", query_args["first_name"]
        ).strip()
        return query_args
    ...
    def _scrape_last_court_date(self, soup):
        ...
        last_date_info = list(
            soup.find("a", attrs={"name": "dockets"})
            .find_all("tr", attrs={"valign": "top"})[-1]
            .stripped_strings
        )
        last_date_info[:2] = [
            " ".join(last_date_info[:2])
        ]  # date + time str concatenation
    ...
    def _scrape_scheduled_court_date(self, soup):
        ...
        if len(next_date_info) == 6:  # listed
            next_date_info[1:3] = [
                " ".join(next_date_info[1:3])
            ]  # combine date and time entries of list
        elif len(next_date_info) == 4:  # not listed
            next_date_info.insert(1, None)
        else:
            raise ValueError(
                f"returned list {next_date_info} should be length 4 or 6"
            )
    ...
    def _scrape_case_title(self, soup):
        ...
        # scrape info
        case_title = list(
            soup.select_one('a[name="description"] > table')
            .select("tr")[0]
            .stripped_strings
        )
        case_title = [case_title[1].replace("\n", "")]
        scrape_dict = dict(
            zip(scheduled_headings, case_title)
        )  # zip together dict
        return scrape_dict
    ...
    def get_case(self, message):
        if "first_name" in message:
            ...
            scrape_dict = [
                {**d, "applications_record_id": message["record_id"]}
                for d in scrape_dict
            ]
        else:
...
def test_name_search():
    ...
    query_args = {
        "last_name": os.getenv("TEST_LAST_NAME_MATCH"),
        "first_name": os.getenv("TEST_FIRST_NAME_MATCH"),
    }
    ...
    query_args = {
        "last_name": os.getenv("TEST_LAST_NAME_MULTIPLE_CASES"),
        "first_name": os.getenv("TEST_FIRST_NAME_MULTIPLE_CASES"),
    }
    info0 = scr.scrape_by_name(query_args)
    print(info0)
